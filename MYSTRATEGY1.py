...
class Strategy():
    ...
    def trade(self, information):
        ...
        if USDT_asset > close_price:
            if current_cross == self.UP and self.last_cross_status == self.DOWN and \
                close_price > l_kama and mom > self.N:
                Log('buy(1)')
            if current_cross == self.DOWN and close_price > l_kama * 0.99 and mom > self.N:
                Log('buy(3)')
            if close_price < l_kama * 0.98 and mom > self.N:
                Log('buy(4)')
            if (current_cross == self.UP and self.last_cross_status == self.DOWN and mom > self.N) or \
                (current_cross == self.DOWN and close_price > l_kama * 0.99 and mom > self.N) or \
            ...
            if current_cross == self.UP and close_price < l_kama * 1.01 and mom < -self.N:
                Log('sell(3)')
            if close_price > l_kama * 1.02 and mom < -self.N:
                Log('sell(4)')
            if ( current_cross == self.DOWN and self.last_cross_status == self.UP and \
                close_price < l_kama and mom < -self.N) or  \
                (current_cross == self.UP and close_price < l_kama * 1.01 and mom < -self.N) or \
